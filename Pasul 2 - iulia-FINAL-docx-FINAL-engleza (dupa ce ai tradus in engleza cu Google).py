...
def format_body(body):
    ...
    for paragraph in body:
        ...
        if numbered_paragraph:
            ...
            # Dacă nu există numerotare, procesăm paragraful normal
...
def capitalize_title(title):
    # Split the title into words and capitalize each word
    words = title.split()
...
# The update_meta_description function remains the same
...
def format_numbered_paragraphs(content):
    # Înlocuire pentru paragrafele numerotate cu `<span class="text_obisnuit2">` pentru numere
    content = re.sub(
        r'<p class="text_obisnuit"><strong>(\d+\.)</strong>',
        r'<p class="text_obisnuit"><span class="text_obisnuit2">\1 </span>',
        content
    )

    # Formatează linkurile pentru a fi clickabile și să fie pe un rând separat
    content = re.sub(
        r'(\bhttps?://[^\s]+)',
        r'<p class="text_obisnuit"><a href="\1">\1</a></p>',
        content
    )
